...
db.products.find({ category: "mobile" }).pretty();
db.products.findOne({ category: "mobile" }).pretty();
...
urgentorderCursor.forEach(function (urgentOrder) {
  print(
    `OrderID: ${urgentOrder._id}, ProductName: ${urgentOrder.productName}, status: ${urgentOrder.status},Quantity: ${urgentOrder.quantity}`
  );
});
// count
